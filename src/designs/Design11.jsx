// Design 16: CAMPO VERDE
// Aesthetic: Lush olive-green field with steel blue accents and icy white text.
// A verdant, nature-inspired scoreboard where the green IS the canvas.
...
const FlipDigit = ({ digit, delay = 0 }) => (
  <motion.div
    ...
    className="relative bg-[#1e2618] text-[#fbfeff] font-display text-xl md:text-2xl
...
const ScoreDisplay = ({ home, away, isLive }) => {
  ...
  return (
    <div className="flex items-center gap-2 md:gap-3">
      ...
      <span className="text-[#fbfeff] text-xl md:text-2xl font-display">
        -
      </span>
      <div className="flex gap-0.5 md:gap-1">
...
const MatchEventsPanel = ({ events }) => {
  ...
  return (
    <div className="border-t-2 border-[#2d5f81]/40">
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 flex items-center justify-end gap-1 md:gap-2">
                {isHome && (
                  event.type === 'substitution' ? (
                    ...
                    <span className="font-condensed text-[#fbfeff] text-xs md:text-sm text-right">
                      {event.player}
                    </span>
                  )
                )}
              ...
              <div className="w-9 md:w-12 flex justify-center">
                <span className="font-condensed text-[#a8c464] text-xs md:text-sm font-semibold">
                  {event.minute}'
                </span>
              </div>
              ...
              <div className="flex-1 flex items-center justify-start gap-1 md:gap-2">
                {!isHome && (
                  event.type === 'substitution' ? (
                    ...
                    <span className="font-condensed text-[#fbfeff] text-xs md:text-sm">
                      {event.player}
                    </span>
                  )
                )}
  ...
  return (
    <div
      className="relative rounded-none md:rounded-xl overflow-hidden
                 border-x-0 md:border-x-2 border-y border-[#2d5f81]/40 md:shadow-lg
                 bg-gradient-to-br from-[#3a4a28] via-[#4a5d35] to-[#2f3e20]"
    >
      ...
      <div className="flex items-stretch">
        ...
        <div className="relative z-10 w-12 md:w-20 shrink-0 flex flex-col items-center justify-center
                        border-r-2 border-[#2d5f81]/40 py-2 md:py-3">
          {isLive ? (
        ...
        <div className="flex-1 relative min-w-0">
          ...
          <div className="relative z-10 flex items-center justify-center gap-1.5 md:gap-4 px-2 py-2 md:px-4 md:py-3">
            ...
            <div className="flex-1 flex flex-col items-center md:flex-row md:items-center md:justify-end gap-0.5 md:gap-2 min-w-0">
              ...
              />
              <div className="font-display text-[#fbfeff] text-xs md:text-lg lg:text-xl tracking-wide md:order-1 text-center md:text-right leading-tight md:truncate">
                {homeTeam?.name}
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center md:flex-row md:items-center md:justify-start gap-0.5 md:gap-2 min-w-0">
              ...
              />
              <div className="font-display text-[#fbfeff] text-xs md:text-lg lg:text-xl tracking-wide text-center md:text-left leading-tight md:truncate">
                {awayTeam?.name}
              </div>
            </div>
          </div>
...
const LeagueSection = ({ leagueId, matchList }) => {
  ...
  return (
    <div className="mb-3 md:mb-6">
      ...
      <div className="border-2 border-[#2d5f81]/40 rounded-lg overflow-hidden
                      bg-[#1e2618]
                      shadow-[0_4px_20px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(45,95,129,0.1)]">

        {/* Header row - integrated into the table */}
        <div className="flex items-center gap-3 px-4 py-3
                        bg-gradient-to-r from-[#2d5f81] via-[#2a5474] to-[#2d5f81]
                        border-b-2 border-[#2d5f81]/50">
          {league?.logo ? (
          ...
          <h3 className="font-display text-[#fbfeff] text-xl tracking-wide
                         drop-shadow-[0_1px_0_rgba(0,0,0,0.3)]">
            {league?.name}
          </h3>
          <div className="flex-1 h-px bg-gradient-to-r from-[#fbfeff]/20 to-transparent ml-2" />
          <span className="font-condensed text-[#fbfeff]/70 text-xs tracking-wider">
        ...
        return (
          <motion.button
            ...
            className={`
              ...
              ${isSelected
                ? 'bg-[#2d5f81] text-[#fbfeff] shadow-lg'
                : 'bg-[#1e2618]/50 text-[#fbfeff]/60 hover:bg-[#1e2618]/80'}
              ${isToday && !isSelected ? 'ring-2 ring-[#2d5f81]/50' : ''}
            `}
          >
...
const LeaguesSidebar = ({ selectedLeague, onLeagueSelect }) => {
  ...
  return (
    <div className="bg-[#1e2618]/80 rounded-lg p-4 border-2 border-[#2d5f81]/20">
      <div className="mb-6 border-2 border-[#2d5f81]/30 rounded-lg overflow-hidden">
        <div className="flex items-center gap-2 text-[#76834e] py-2.5 px-3 bg-[#2d5f81]/20">
          <span className="font-display text-lg uppercase tracking-wide font-semibold">Destacados</span>
        </div>
        <div className="divide-y divide-[#2d5f81]/20">
          {featured.map(league => (
            <button
              key={league.id}
              ...
              className={`
                ...
                ${selectedLeague === league.id
                  ? 'bg-[#2d5f81] text-[#fbfeff] font-semibold'
                  : 'text-white/90 hover:bg-[#2d5f81]/20'}
              `}
            >
              <motion.span
                whileHover={{ x: 4 }}
      ...
      <div className="border-t border-[#2d5f81]/20 pt-4">
        <h4 className="font-display text-[#fbfeff]/60 text-base mb-3 tracking-wide">
          POR PAIS
        </h4>
        {countries.map(country => {
          ...
          return (
            <div key={country.id} className="mb-3 border border-[#2d5f81]/20 rounded-lg overflow-hidden">
              <div className="flex items-center gap-2 text-[#76834e] text-sm py-2 px-3 bg-[#2d5f81]/10">
                ...
                {countryLeagues.map(league => (
                  <button
                    ...
                    className={`
                      ...
                      ${selectedLeague === league.id
                        ? 'text-[#76834e] bg-[#2d5f81]/10 font-semibold'
                        : 'text-white/90 hover:bg-[#2d5f81]/20'}
                    `}
                  >
                    <motion.span
...
export default function Design11({ showLiveTicker = true, tickerPosition = 'top' }) {
  ...
  return (
    <div className="min-h-screen bg-[#76834e]">

      {/* Header */}
      <header className="relative overflow-hidden border-b-2 border-[#2d5f81]">
        {/* Light beams */}
        <div className="absolute inset-0">
          <div className="absolute top-0 left-1/4 w-32 h-full bg-gradient-to-b from-[#2d5f81]/10 to-transparent
                          transform -skew-x-12" />
          <div className="absolute top-0 right-1/3 w-24 h-full bg-gradient-to-b from-[#fbfeff]/5 to-transparent
                          transform skew-x-6" />
        </div>
          ...
          >
            ...
            <div className="flex items-center gap-4">
              <PizarraLogo
                ...
                animated={false}
              />
              <h1 className="font-display text-[#fbfeff] text-5xl md:text-6xl tracking-wider
            ...
            <p className="font-condensed text-[#fbfeff]/50 text-[10px] md:text-xs tracking-[0.4em] uppercase -mt-1 md:mt-1">
      ...
      {showLiveTicker && tickerPosition === 'top' && liveMatches.length > 0 && (
        <div className="bg-[#2d5f81] border-y-2 border-[#76834e]/50 overflow-hidden">
          <div className="flex items-stretch relative">
            <div className="bg-[#1e2618] px-4 flex items-center gap-2 shrink-0 relative z-10">
              <motion.div
              ...
              {[...liveMatches, ...liveMatches].map((match, idx) => {
                ...
                return (
                  <span key={idx} className="font-condensed text-white text-sm flex items-center">
                    {/* Separador vertical */}
                    <span className="h-6 w-px bg-[#fbfeff]/30 mx-8" />
                    {/* Minuto */}
                    <span className="text-[#fbfeff]/60 mr-3">{match.minute}'</span>
                    {/* Contenido del partido */}
                    {home?.name}
                    <img src={home?.logo} alt="" className="w-5 h-5 object-contain mx-1.5" />
                    <span className="font-bold text-[#fbfeff]">{match.homeScore} - {match.awayScore}</span>
      ...
      <main className="relative">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-2 md:px-4 pt-0 pb-2 md:py-8">
          <div className="flex flex-col lg:flex-row gap-3 lg:gap-8">
            ...
            <MobileSidebar variant="campo" selectedLeague={selectedLeague} bottomOffset={tickerPosition === 'bottom' && showLiveTicker && liveMatches.length > 0}>
              <LeaguesSidebar
                selectedLeague={selectedLeague}
                onLeagueSelect={(id) => setSelectedLeague(id === selectedLeague ? null : id)}
              />
            ...
            <div className="flex-1 lg:order-2">
              ...
              {Object.keys(groupedMatches).length === 0 && (
                <div className="text-center py-16">
                  <Landmark size={64} className="mx-auto mb-4 text-[#fbfeff]/40" />
                  <p className="font-display text-[#fbfeff]/40 text-xl">
                    NO HAY PARTIDOS
                  </p>
                </div>
              )}
      ...
      {/* Footer */}
      <footer className={`border-t-2 border-[#2d5f81]/30 mt-12 py-6 ${tickerPosition === 'bottom' && showLiveTicker && liveMatches.length > 0 ? 'pb-16' : ''}`}>
        <div className="max-w-7xl mx-auto px-4 text-center">
          <p className="font-condensed text-[#fbfeff]/30 text-sm tracking-wider">
      ...
      {showLiveTicker && tickerPosition === 'bottom' && liveMatches.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-[#2d5f81] border-t-2 border-[#76834e]/50 overflow-hidden shadow-[0_-4px_20px_rgba(0,0,0,0.5)]">
          <div className="flex items-stretch relative">
            <div className="bg-[#1e2618] px-4 flex items-center gap-2 shrink-0 relative z-10">
              <motion.div
                animate={{ scale: [1, 1.2, 1] }}
