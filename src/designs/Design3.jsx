// Design 3: SLATE CHALK BOARD
// Aesthetic: Dark slate monochromatic with emerald accents exclusively for live indicators.
// Clean, modern dark UI with chalk-like textures on a deep blue-gray canvas.
...
const FlipDigit = ({ digit, delay = 0 }) => (
  <motion.div
    ...
    className="relative bg-slate-900 text-white font-display text-xl md:text-2xl
...
const ScoreDisplay = ({ home, away, isLive }) => {
  ...
  return (
    <div className="flex items-center gap-2 md:gap-3">
      ...
      <span className="text-slate-600 text-xl md:text-2xl font-display">
        -
      </span>
      <div className="flex gap-0.5 md:gap-1">
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 flex items-center justify-end gap-1 md:gap-2">
                {isHome && (
                  event.type === 'substitution' ? (
                    ...
                    <span className="font-condensed text-white text-xs md:text-sm text-right">
                      {event.player}
                    </span>
                  )
                )}
              ...
              <div className="w-9 md:w-12 flex justify-center">
                <span className="font-condensed text-slate-400 text-xs md:text-sm font-semibold">
                  {event.minute}'
                </span>
              </div>
              ...
              <div className="flex-1 flex items-center justify-start gap-1 md:gap-2">
                {!isHome && (
                  event.type === 'substitution' ? (
                    ...
                    <span className="font-condensed text-white text-xs md:text-sm">
                      {event.player}
                    </span>
                  )
                )}
  ...
  return (
    <div
      className={`relative rounded-none md:rounded-xl overflow-hidden
                 border-x-0 md:border-x border-y border-slate-600/30 md:shadow-lg
                 bg-emerald-500/20 backdrop-blur-sm
                 ${isLive ? 'md:border-emerald-500/50' : ''}`}
    >
      {/* Main row */}
      <div className="flex items-stretch">
        {/* Time/Status section */}
        <div className="relative z-10 w-12 md:w-20 shrink-0 flex flex-col items-center justify-center
                        border-r border-slate-600/30 py-2 md:py-3">
          {isLive ? (
            <span className="font-condensed text-emerald-400 font-bold text-base md:text-2xl">
              {match.minute}'
            </span>
          ) : isFinished ? (
        ...
        <div className="flex-1 relative min-w-0">
          <div className="relative z-10 flex items-center justify-center gap-1.5 md:gap-4 px-2 py-2 md:px-4 md:py-3">
            ...
            <div className="flex-1 flex flex-col items-center md:flex-row md:items-center md:justify-end gap-0.5 md:gap-2 min-w-0">
              ...
              />
              <div className="font-display text-white text-xs md:text-lg lg:text-xl tracking-wide md:order-1 text-center md:text-right leading-tight md:truncate">
                {homeTeam?.name}
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center md:flex-row md:items-center md:justify-start gap-0.5 md:gap-2 min-w-0">
              ...
              />
              <div className="font-display text-white text-xs md:text-lg lg:text-xl tracking-wide text-center md:text-left leading-tight md:truncate">
                {awayTeam?.name}
              </div>
            </div>
          </div>
    ...
    <div className="mb-3 md:mb-6">
      <div className="border border-slate-600/30 rounded-lg overflow-hidden
        ...
        <div className="flex items-center gap-3 px-4 py-3
          ...
          <div className="flex-1 h-px bg-gradient-to-r from-slate-600/30 to-transparent ml-2" />
          <span className="font-condensed text-slate-500 text-xs tracking-wider">
        ...
        return (
          <motion.button
            ...
            className={`
              ...
              ${isSelected
                ? 'bg-slate-700/50 text-white shadow-lg border border-slate-400/50'
                : 'bg-slate-800/50 text-slate-500 hover:bg-slate-800/80'}
              ${isToday && !isSelected ? 'ring-2 ring-slate-500/40' : ''}
            `}
          >
...
const LeaguesSidebar = ({ selectedLeague, onLeagueSelect }) => {
  ...
  return (
    <div className="bg-slate-900/80 rounded-lg p-4 border border-slate-600/30">
      <div className="mb-6 border border-slate-600/30 rounded-lg overflow-hidden">
        <div className="flex items-center gap-2 text-white py-2.5 px-3 bg-slate-800/60">
            ...
            <button
              ...
              className={`
                ...
                ${selectedLeague === league.id
                  ? 'bg-slate-700/50 text-white font-semibold border-l-2 border-slate-400/50'
      ...
      <div className="border-t border-slate-600/30 pt-4">
        <h4 className="font-display text-slate-500 text-base mb-3 tracking-wide">
          POR PAÍS
        </h4>
        {countries.map(country => {
          ...
          return (
            <div key={country.id} className="mb-3 border border-slate-600/30 rounded-lg overflow-hidden">
              <div className="flex items-center gap-2 text-slate-400 text-sm py-2 px-3 bg-slate-800/40">
...
export default function Design3({ showLiveTicker = true, tickerPosition = 'top' }) {
  ...
  return (
    <div className="min-h-screen bg-slate-900">
      ...
      {/* Header */}
      <header className="relative overflow-hidden border-b border-slate-600/30">
        {/* Subtle light beams */}
        <div className="absolute inset-0">
          <div className="absolute top-0 left-1/4 w-32 h-full bg-gradient-to-b from-slate-600/10 to-transparent
                          transform -skew-x-12" />
          <div className="absolute top-0 right-1/3 w-24 h-full bg-gradient-to-b from-slate-600/5 to-transparent
                          transform skew-x-6" />
        </div>
          ...
          >
            ...
            <div className="flex items-center gap-4">
              <PizarraLogo
                ...
                animated={false}
              />
              <h1 className="font-display text-white text-5xl md:text-6xl tracking-wider
            ...
            {/* Tagline */}
            <p className="font-condensed text-slate-500 text-[10px] md:text-xs tracking-[0.4em] uppercase -mt-1 md:mt-1">
      ...
      {showLiveTicker && tickerPosition === 'top' && liveMatches.length > 0 && (
        <div className="bg-emerald-500/20 border-y border-emerald-500/40 overflow-hidden">
          <div className="flex items-stretch relative">
            <div className="bg-slate-900 px-4 flex items-center gap-2 shrink-0 relative z-10">
              <motion.div
                animate={{ scale: [1, 1.2, 1] }}
                transition={{ duration: 0.5, repeat: Infinity }}
                className="w-2 h-2 rounded-full bg-emerald-400"
              />
              <span className="font-display text-emerald-400 text-sm tracking-wider">EN VIVO</span>
            </div>
            <motion.div
              animate={{ x: [0, -2000] }}
              ...
              {[...liveMatches, ...liveMatches].map((match, idx) => {
                ...
                return (
                  <span key={idx} className="font-condensed text-white text-sm flex items-center">
                    <span className="h-6 w-px bg-slate-600/50 mx-8" />
                    <span className="text-emerald-400 mr-3">{match.minute}'</span>
                    {home?.name}
      ...
      <main className="relative">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-2 md:px-4 pt-0 pb-2 md:py-8">
          <div className="flex flex-col lg:flex-row gap-3 lg:gap-8">
            ...
            <MobileSidebar selectedLeague={selectedLeague} bottomOffset={tickerPosition === 'bottom' && showLiveTicker && liveMatches.length > 0} variant="slate">
              <LeaguesSidebar
                selectedLeague={selectedLeague}
                onLeagueSelect={(id) => setSelectedLeague(id === selectedLeague ? null : id)}
              />
            ...
            <div className="flex-1 lg:order-2">
              ...
              {Object.keys(groupedMatches).length === 0 && (
                <div className="text-center py-16">
                  <Landmark size={64} className="mx-auto mb-4 text-slate-700" />
                  <p className="font-display text-slate-700 text-xl">
                    NO HAY PARTIDOS
                  </p>
                </div>
              )}
      ...
      {/* Footer */}
      <footer className={`border-t border-slate-600/30 mt-12 py-6 ${tickerPosition === 'bottom' && showLiveTicker && liveMatches.length > 0 ? 'pb-16' : ''}`}>
        <div className="max-w-7xl mx-auto px-4 text-center">
          <p className="font-condensed text-slate-700 text-sm tracking-wider">
      ...
      {showLiveTicker && tickerPosition === 'bottom' && liveMatches.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-emerald-500/20 border-t border-emerald-500/40 overflow-hidden shadow-[0_-4px_20px_rgba(0,0,0,0.5)]">
          <div className="flex items-stretch relative">
            <div className="bg-slate-900 px-4 flex items-center gap-2 shrink-0 relative z-10">
              <motion.div
                animate={{ scale: [1, 1.2, 1] }}
